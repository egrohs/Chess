...
public class SwingMain extends JFrame implements MouseListener {
	...
	private SwingCell c, previous;
	private int chance = 0;
	private Square boardState[][];
	private List<SwingCell> destinationlist = new ArrayList<SwingCell>();
	...
	private Time timer;
	public static SwingMain Mainboard;
	...
	public static void main(String[] args) throws CloneNotSupportedException {
		...
		Mainboard = new SwingMain();
		Mainboard.setVisible(true);
		Mainboard.setResizable(false);
	}

	// Constructor
	private SwingMain() throws CloneNotSupportedException {
		timeRemaining = 60;
		...
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 8; j++) {
				...
				cell = new SwingCell(new Square(i, j, null/* Piece */));
				cell.addMouseListener(this);
				board.add(cell);
				boardState[i][j] = cell.sq;
			}
	...
	public void changechance() {
		...
		if (!end && timer != null) {
			...
			if (SwingMain.move == "White")
				SwingMain.move = "Black";
			else
				SwingMain.move = "White";
			CHNC.setText(SwingMain.move);
			showPlayer.add(CHNC);
		}
	}
	...
	private void cleandestinations(List<SwingCell> destlist) // Function to clear the last move's destinations
	{
		ListIterator<SwingCell> it = destlist.listIterator();
		while (it.hasNext())
			it.next().removepossibledestination();
	}

	// A function that indicates the possible moves by highlighting the Cells
	private void highlightdestinations(ArrayList<SwingCell> destlist) {
		ListIterator<SwingCell> it = destlist.listIterator();
		while (it.hasNext())
	...
	private boolean willkingbeindanger(SwingCell fromcell, SwingCell tocell) {
		Square newboardstate[][] = new Square[8][8];
		Square cells[][] = new Square[8][8];

		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 8; j++) {
				try {
					newboardstate[i][j] = new Square(boardState[i][j]);
	...
	private ArrayList<SwingCell> filterdestination(ArrayList<SwingCell> destlist, SwingCell fromcell) {
		ArrayList<SwingCell> newlist = new ArrayList<SwingCell>();
		Square newboardstate[][] = new Square[8][8];
		ListIterator<SwingCell> it = destlist.listIterator();
		int x, y;
		while (it.hasNext()) {
	...
	// is under Check
	private List<Square> incheckfilter(List<Square> destlist, Square fromcell, int color) {
		List<Square> newlist = new ArrayList<Square>();
		Square newboardstate[][] = new Square[8][8];
	...
	public boolean checkmate(int color) {
		List<Square> dlist = new ArrayList<Square>();
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
	...
	public void mouseClicked(MouseEvent arg0) {
		...
		c = (SwingCell) arg0.getSource();
		if (previous == null) {
			if (c.sq.getpiece() != null) {
				if (c.sq.getpiece().getcolor() != chance)
					return;
				c.select();
				previous = c;
				destinationlist.clear();
				destinationlist = c.sq.getpiece().move(boardState, c.x, c.y);
				if (c.sq.getpiece() instanceof King)
					destinationlist = filterdestination(destinationlist, c);
				else {
					if (boardState[getKing(chance).getx()][getKing(chance).gety()].ischeck())
						destinationlist = new ArrayList<SwingCell>(filterdestination(destinationlist, c));
			...
			if (c.x == previous.x && c.y == previous.y) {
				...
				destinationlist.clear();
				previous = null;
			} else if (c.sq.getpiece() == null || previous.sq.getpiece().getcolor() != c.sq.getpiece().getcolor()) {
				if (c.sq.ispossibledestination()) {
					if (c.sq.getpiece() != null)
						c.removePiece();
					c.setPiece(previous.sq.getpiece());
					if (previous.sq.ischeck())
					...
					if (c.sq.getpiece() instanceof King) {
						((King) c.sq.getpiece()).setx(c.x);
						((King) c.sq.getpiece()).sety(c.y);
					}
					changechance();
					if (!end) {
						timer.reset();
				...
				cleandestinations(destinationlist);
				destinationlist.clear();
			} else if (previous.sq.getpiece().getcolor() == c.sq.getpiece().getcolor()) {
				previous.deselect();
				cleandestinations(destinationlist);
				destinationlist.clear();
				c.select();
		...
		if (c.sq.getpiece() != null && c.sq.getpiece() instanceof King) {
			((King) c.sq.getpiece()).setx(c.x);
			((King) c.sq.getpiece()).sety(c.y);
		}
	}
