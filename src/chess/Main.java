...
public class Main extends JFrame implements MouseListener
{
		...
		for(int i=0;i<8;i++)
			...
			{	
				...
				P=bp[j];
					...
					P=wp[j];
				cell=new Square(i,j,P);
				cell.painel.addMouseListener(this);
				board.add(cell.painel);
				boardState[i][j]=cell;
			}
    ...
    private void cleandestinations(ArrayList<Square> destlist)      //Function to clear the last move's destinations
    {
	...
	public void mouseClicked(MouseEvent arg0){
		...
		if (previous==null)
		{
			if(c.getpiece()!=null)
			{
				...
				{
					if(boardState[getKing(chance).getx()][getKing(chance).gety()].ischeck())
						destinationlist = new ArrayList<Square>(filterdestination(destinationlist,c));
